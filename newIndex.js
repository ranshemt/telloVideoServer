...
udpClient.send("command", 0, 7, commandPort, telloIP, (e) => {
  if (e) {
    console.log(e);
  } else {
    console.log("command sent to drone");
  }
});

sleep(2500).then(() => {
  udpClient.send("streamon", 0, 8, commandPort, telloIP, (e) => {
    if (e) {
      console.log(e);
    } else {
      console.log("streamon sent to drone");
      console.log("setting command setInterval");
      setInterval(() => {
        //
        udpClient.send("command", 0, 7, commandPort, telloIP, (e) => {
          if (e) {
...
udpClient.on("message", (data) => {
  console.log("drone.on: " + data);
  if (data == "ok" && !isFirstOK) {
    console.log("first OK! sending streamon");
    isFirstOK = true;
    udpClient.send("streamon", 0, 8, commandPort, telloIP, (e) => {
      if (e) {
        console.log("streamon send error: " + e);
      } else {
        console.log("streamon sent to drone");
      }
    });
